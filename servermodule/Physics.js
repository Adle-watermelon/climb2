...
export function updatePlayerMovement(player, keystate, delta, chunkmanager) {
    ...
    const wallStickTime = 0.6;    // 壁に粘着できる最大時間（秒）
    ...
    if (keystate['KeyW']) {
        if ((player.OnGround || player.OnWall) && player.jumpinterval == 0.0) {
            ...
            // 壁ジャンプ時に横方向へ押し出すとより自然
            if(player.OnWall){
                if (player.direction === 'left') {
                    player.vx = moveSpeed * 2.0;
                } else if (player.direction === 'right') {
                    player.vx = -moveSpeed * 2.0;
                }
                player.vy = -initialJumpVelocity * 0.75;
            }
    ...
    // 当たり判定
        if(chunkmanager.getBlock(player.x + charawidth/2,player.y - charaheight/2) !='air' || chunkmanager.getBlock(player.x + charawidth/2,player.y - charaheight) !='air' || chunkmanager.getBlock(player.x + charawidth/2,player.y - 0.01) !='air'){
            ...
            player.vx = 0.0;
            if(chunkmanager.getBlock(player.x + charawidth/2, player.y - charaheight/1.2) != 'air'){
                player.OnWall = true;
            }
        }else if(chunkmanager.getBlock(player.x - charawidth/2,player.y - charaheight/2) !='air' || chunkmanager.getBlock(player.x - charawidth/2,player.y - charaheight) !='air' || chunkmanager.getBlock(player.x - charawidth/2,player.y - 0.01) !='air'){
            player.x = Math.floor(player.x) + charawidth/2
            player.vx = 0.0
            if(chunkmanager.getBlock(player.x - charawidth/2 - 0.01, player.y - charaheight/1.2) != 'air'){
                player.OnWall = true;
            }
        }else {
            player.OnWall = false;
